...
import 'package:trendy_chikitsa/models/client_responses/client_review_list_response.dart';
...
class ClientReviewList extends StatefulWidget {
  String from_page = "", token = "";

  ClientReviewList({super.key});

  @override
  ClientReviewListState createState() => ClientReviewListState();
...
class ClientReviewListState extends State<ClientReviewList>
...
/*  @override
  ...
  Widget build(BuildContext context) {
    return Sizer(builder: (context, orientation, deviceType) {
      return Scaffold(
          ...
          body: FutureBuilder<ClientReviewListResponse?>(
            future: getHealerReviews(),
            builder: (context, snapshot) {
              if (snapshot.connectionState == ConnectionState.done) {
                print('snapshot--  ${snapshot.error}');
                if (_ClientReviewListResponse != null &&
                    _ClientReviewListResponse!.response!.isNotEmpty) {
                  print('snapshot--  ${snapshot.error}');
                  //   print('value is..  ${new Map<String, dynamic>.from(snapshot.data).}');

                  return ListView.builder(
                      itemCount: _ClientReviewListResponse!.response!.length,
                      primary: false,
  ...
  ClientReviewListResponse? _ClientReviewListResponse;

  Future<ClientReviewListResponse?> getHealerReviews() async {
    var data;
    var request = http.MultipartRequest('POST', BaseuURL.client_rating);
    ...
    if (response.statusCode == 200) {
      ...
      if (data["status"] == "true" && data["msg"] == "success") {
        ...
        _ClientReviewListResponse =
            ClientReviewListResponse.fromJson(jsonResponse);
    ...
    return _ClientReviewListResponse;
